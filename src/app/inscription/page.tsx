        ...
        <div className="relative max-w-md w-full bg-surface border border-white/6 rounded-2xl glow-green p-8 md:p-12 text-center overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="w-14 h-14 rounded-full bg-spruce-light/20 border border-spruce-light/30 flex items-center justify-center mx-auto mb-6">
              <div className="w-5 h-5 rounded-full bg-brass-light" />
            </div>
            ...
            <div className="bg-void border border-white/6 rounded-xl p-4 mb-10 inline-block">
              <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-paper/30 mb-1.5">
                Référence dossier
              </div>
              <div className="font-mono text-base md:text-lg font-medium text-brass-light">
                {refCode}
              </div>
            </div>
            ...
            <div className="absolute -bottom-10 -right-6 opacity-60">
              <StampMark />
            </div>
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <h1 className="font-display text-4xl md:text-5xl mt-2 leading-[1.05] text-paper">
            Formulaire<br />
            <span className="italic text-brass-light">d&apos;inscription.</span>
          </h1>
        ...
        <div className="relative bg-surface border border-white/6 rounded-2xl overflow-hidden">
          ...
          <div className="px-6 md:px-8 pt-8 pb-5 border-b border-white/5">
            <div className="flex items-start justify-between">
              <div>
                <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-paper/25">
                  Bordereau d&apos;inscription
                </div>
                <div className="font-display text-lg mt-0.5 text-paper">
                  Semou Group <span className="text-brass-light">× CFA CUSEMS</span>
                </div>
              </div>
          ...
          <form onSubmit={handleSubmit} className="px-6 md:px-8 pb-8">
                ...
                <div className="flex items-center gap-6 py-0.5">
                  {(['FONCTIONNAIRE', 'CONTRACTUEL'] as const).map(t => (
                    <label key={t} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="typeAgent" value={t}
                        checked={typeAgent === t} onChange={() => setTypeAgent(t)}
            ...
            <Field n="07" label="Académie (IA)">
              <input value={ia} onChange={e => setIa(e.target.value)}
                placeholder="ex : Kolda"
                className="w-full bg-transparent border-b border-white/10 focus:border-brass outline-none font-body text-sm text-paper pb-0.5 transition-colors placeholder:text-paper/15" />
            </Field>
            <Field n="08" label="IEF">
              <input value={ief} onChange={e => setIef(e.target.value)}
            ...
            <Field n="10" label="École / Poste">
              <input value={ecole} onChange={e => setEcole(e.target.value)}
            ...
            <FileField n="11" label="CNI · Recto" file={cniRecto}
              onPick={() => rectoRef.current?.click()}
              onClear={() => setCniRecto(null)} />
            <input ref={rectoRef} type="file" accept="image/*,.pdf" className="hidden"
              onChange={e => setCniRecto(e.target.files?.[0] ?? null)} />

            <FileField n="12" label="CNI · Verso" file={cniVerso}
              onPick={() => versoRef.current?.click()}
              onClear={() => setCniVerso(null)} />
            <input ref={versoRef} type="file" accept="image/*,.pdf" className="hidden"
              onChange={e => setCniVerso(e.target.files?.[0] ?? null)} />

            <FileField n="13" label="Bulletin salaire" file={bulletin}
              onPick={() => bulletRef.current?.click()}
              onClear={() => setBulletin(null)} />
            <input ref={bulletRef} type="file" accept="image/*,.pdf" className="hidden"
        ...
        {file ? (
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={onClear}
              className="font-mono text-sm text-paper/20 hover:text-clay transition-colors flex-shrink-0">×</button>
          </div>
        ) : (
